...
- `enrollment_site_id` (String) The Jamf Pro Site ID that computers will be added to during enrollment. Default is -1.
...
- `site_id` (String) The jamf pro site ID.
...
- `hidden_admin_account` (Boolean) Indicates if the admin account is hidden. Default is false.
- `local_admin_account_enabled` (Boolean) Indicates if the local admin account is enabled. Default is false.
- `local_user_managed` (Boolean) Indicates if the local user is managed. Default is false.
...
- `prefill_primary_account_info_feature_enabled` (Boolean) Indicates if prefilling primary account info feature is enabled. Default is false.
- `prefill_type` (String) Pre-fill primary account information type (CUSTOM or DEVICE_OWNER). Default is CUSTOM.
- `prevent_prefill_info_from_modification` (Boolean) Lock prefill primary account information from modification. Default is false.
- `user_account_type` (String) Type of user account (ADMINISTRATOR, STANDARD, SKIP). Default is ADMINISTRATOR.

Read-Only:

- `id` (String) ID of Account Settings.
- `payload_configured` (Boolean) Indicates if the payload is configured. Always true.
