package mobilesextensionattributes
...
func construct(d *schema.ResourceData) (*jamfpro.ResourceMobileExtensionAttribute, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to marshal Jamf Pro Mobile Extension Attribute to JSON: %v", err)
	}

	log.Printf("[DEBUG] Constructed Jamf Pro Mobile Extension Attribute JSON:\n%s\n", string(resourceJSON))
