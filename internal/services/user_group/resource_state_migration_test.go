...
func setFixture() map[string]any {
	return map[string]any{"name": "set-test", "site_id": -1, "assigned_user_ids": []any{3, 1, 2}, "user_additions": []any{map[string]any{"id": "5", "username": "bravo"}, map[string]any{"id": "4", "username": "alpha"}}, "user_deletions": []any{map[string]any{"id": "7", "username": "delta"}, map[string]any{"id": "6", "username": "charlie"}}}
}
func TestStateUpgradeProtocol(t *testing.T) {
	current := ResourceJamfProUserGroups()
	require.Equal(t, 1, current.SchemaVersion)
	require.Len(t, current.StateUpgraders, 1)
	server := schema.NewGRPCProviderServer(&schema.Provider{ResourcesMap: map[string]*schema.Resource{"jamfpro_user_group": current}})
			...
			for _, flatmap := range []bool{false, true} {
				t.Run(fmt.Sprintf("v%d/flatmap=%t/empty=%t", version, flatmap, empty), func(t *testing.T) {
					...
					if empty {
						...
						item := input["user_additions"].([]any)[0].(map[string]any)
						item["full_name"] = "Preserved name"
						item["phone_number"] = "000-0000"
						item["email_address"] = "test@example.invalid"
					}
					...
					response, err := server.UpgradeResourceState(context.Background(), &tfprotov5.UpgradeResourceStateRequest{TypeName: "jamfpro_user_group", Version: int64(version), RawState: raw})
					require.NoError(t, err)
					...
					require.NoError(t, err)
					normalized, err := server.UpgradeResourceState(context.Background(), &tfprotov5.UpgradeResourceStateRequest{TypeName: "jamfpro_user_group", Version: 1, RawState: &tfprotov5.RawState{JSON: wantJSON}})
					require.NoError(t, err)
...
func TestSetReorderingAndChanges(t *testing.T) {
	for _, tc := range []struct {
		...
		{"legacy list order", resourceV0(), true}, {"set order", ResourceJamfProUserGroups(), false},
	} {
...
func TestSetMembershipChanges(t *testing.T) {
	for _, key := range []string{"assigned_user_ids", "user_additions", "user_deletions"} {
