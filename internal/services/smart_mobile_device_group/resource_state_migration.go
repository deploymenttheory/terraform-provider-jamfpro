...
var _ resource.ResourceWithUpgradeState = &smartMobileDeviceGroupFrameworkResource{}

// UpgradeState returns the state upgraders for the resource
func (r *smartMobileDeviceGroupFrameworkResource) UpgradeState(ctx context.Context) map[int64]resource.StateUpgrader {
...
func upgradeStateV0toV1(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
	...
	upgradedStateData := smartMobileDeviceGroupResourceModel{
	...
	if !priorStateData.Criteria.IsNull() && !priorStateData.Criteria.IsUnknown() {
		...
		upgradedStateData.Criteria = make([]smartMobileDeviceGroupCriteriaDataModel, 0, len(criteriaV0List))
		for _, oldCriteria := range criteriaV0List {
			newCriteria := smartMobileDeviceGroupCriteriaDataModel(oldCriteria)
			upgradedStateData.Criteria = append(upgradedStateData.Criteria, newCriteria)
		}
	} else {
		upgradedStateData.Criteria = []smartMobileDeviceGroupCriteriaDataModel{}
	}
