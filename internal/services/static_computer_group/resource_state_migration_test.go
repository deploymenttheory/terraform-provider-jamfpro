...
func setFixture() map[string]any {
	return map[string]any{"name": "set-test", "site_id": -1, "assigned_computer_ids": []any{3, 1, 2}}
}
func TestStateUpgradeProtocol(t *testing.T) {
	current := ResourceJamfProStaticComputerGroups()
	require.Equal(t, 1, current.SchemaVersion)
	require.Len(t, current.StateUpgraders, 1)
	server := schema.NewGRPCProviderServer(&schema.Provider{ResourcesMap: map[string]*schema.Resource{"jamfpro_static_computer_group": current}})
		...
		for _, empty := range []bool{false, true} {
			for _, flatmap := range []bool{false, true} {
				t.Run(fmt.Sprintf("v%d/flatmap=%t/empty=%t", version, flatmap, empty), func(t *testing.T) {
					...
					response, err := server.UpgradeResourceState(context.Background(), &tfprotov5.UpgradeResourceStateRequest{TypeName: "jamfpro_static_computer_group", Version: int64(version), RawState: raw})
					require.NoError(t, err)
					...
					require.NoError(t, err)
					normalized, err := server.UpgradeResourceState(context.Background(), &tfprotov5.UpgradeResourceStateRequest{TypeName: "jamfpro_static_computer_group", Version: 1, RawState: &tfprotov5.RawState{JSON: wantJSON}})
					require.NoError(t, err)
...
func TestSetReorderingAndChanges(t *testing.T) {
	for _, tc := range []struct {
		...
		{"legacy list order", resourceV0(), true}, {"set order", ResourceJamfProStaticComputerGroups(), false},
	} {
...
func TestExportOfflineFixture(t *testing.T) {
	...
	require.NoError(t, err)
	state := map[string]any{"version": 4, "terraform_version": "1.14.6", "serial": 1, "lineage": "00000000-0000-4000-8000-000000000042", "outputs": map[string]any{}, "resources": []any{map[string]any{"mode": "managed", "type": "jamfpro_static_computer_group", "name": "test", "provider": "provider[\"registry.terraform.io/deploymenttheory/jamfpro\"]", "instances": []any{map[string]any{"schema_version": 0, "attributes": json.RawMessage(attrs), "sensitive_attributes": []any{}}}}}}
	encoded, err := json.MarshalIndent(state, "", "  ")
	require.NoError(t, err)
	require.NoError(t, os.WriteFile(filepath.Join(dir, "terraform.tfstate"), encoded, 0600))
	reverseCollections(input)
	config := map[string]any{"resource": map[string]any{"jamfpro_static_computer_group": map[string]any{"test": input}}}
...
func TestConstructorSetMembershipAndOmittedConfig(t *testing.T) {
	r := ResourceJamfProStaticComputerGroups()
	for _, input := range []map[string]any{setFixture(), {"name": "empty", "assigned_computer_ids": []any{}}} {
		data := schema.TestResourceDataRaw(t, r.Schema, input)
		data.SetId("42")
		state := data.State()
		...
		if len(input["assigned_computer_ids"].([]any)) == 0 {
			configAttributes["assigned_computer_ids"] = cty.SetValEmpty(cty.Number)
		}
		state.RawConfig = cty.ObjectVal(configAttributes)
		payload, err := construct(r.Data(state))
		require.NoError(t, err)
		require.NotNil(t, payload.Computers)
		require.Len(t, *payload.Computers, len(input["assigned_computer_ids"].([]any)))
		attributes := raw.AsValueMap()
		attributes["assigned_computer_ids"] = cty.NullVal(raw.GetAttr("assigned_computer_ids").Type())
		state.RawConfig = cty.ObjectVal(attributes)
		payload, err = construct(r.Data(state))
		require.NoError(t, err)
		require.Nil(t, payload.Computers)
	}
}
