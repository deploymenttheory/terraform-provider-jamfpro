...
func setFixture() map[string]any {
	return map[string]any{"name": "set-test", "access_level": "Full Access", "privilege_set": "Custom", "member_ids": []any{3, 1, 2}, "jss_objects_privileges": []any{"Read Computers"}}
}
func TestStateUpgradeProtocol(t *testing.T) {
	current := ResourceJamfProAccountGroups()
	require.Equal(t, 1, current.SchemaVersion)
	require.Len(t, current.StateUpgraders, 1)
	server := schema.NewGRPCProviderServer(&schema.Provider{ResourcesMap: map[string]*schema.Resource{"jamfpro_account_group": current}})
		...
		for _, empty := range []bool{false, true} {
			for _, flatmap := range []bool{false, true} {
				t.Run(fmt.Sprintf("v%d/flatmap=%t/empty=%t", version, flatmap, empty), func(t *testing.T) {
					...
					response, err := server.UpgradeResourceState(context.Background(), &tfprotov5.UpgradeResourceStateRequest{TypeName: "jamfpro_account_group", Version: int64(version), RawState: raw})
					require.NoError(t, err)
					...
					require.NoError(t, err)
					normalized, err := server.UpgradeResourceState(context.Background(), &tfprotov5.UpgradeResourceStateRequest{TypeName: "jamfpro_account_group", Version: 1, RawState: &tfprotov5.RawState{JSON: wantJSON}})
					require.NoError(t, err)
...
func TestSetReorderingAndChanges(t *testing.T) {
	for _, tc := range []struct {
		...
		{"legacy list order", resourceV0(), true}, {"set order", ResourceJamfProAccountGroups(), false},
	} {
