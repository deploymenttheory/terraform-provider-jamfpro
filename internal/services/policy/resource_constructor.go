...
func constructSelfService(d *schema.ResourceData, out *jamfpro.ResourcePolicy) {
	if len(d.Get("self_service").([]any)) > 0 {
		...
		// If self service is disabled, only set the use_for_self_service flag
		// and ignore all other self service attributes
		...
		// Self service is enabled, process all attributes
		out.SelfService = jamfpro.PolicySubsetSelfService{
		...
		// Only include icon if icon ID is greater than 0
