...
func setFixture() map[string]any {
	return map[string]any{"name": "set-test", "site_id": -1, "assigned_mobile_device_ids": []any{3, 1, 2}}
}
func TestStateUpgradeProtocol(t *testing.T) {
	current := ResourceJamfProStaticMobileDeviceGroups()
	require.Equal(t, 1, current.SchemaVersion)
	require.Len(t, current.StateUpgraders, 1)
	server := schema.NewGRPCProviderServer(&schema.Provider{ResourcesMap: map[string]*schema.Resource{"jamfpro_static_mobile_device_group": current}})
		...
		for _, empty := range []bool{false, true} {
			for _, flatmap := range []bool{false, true} {
				t.Run(fmt.Sprintf("v%d/flatmap=%t/empty=%t", version, flatmap, empty), func(t *testing.T) {
					...
					response, err := server.UpgradeResourceState(context.Background(), &tfprotov5.UpgradeResourceStateRequest{TypeName: "jamfpro_static_mobile_device_group", Version: int64(version), RawState: raw})
					require.NoError(t, err)
					...
					require.NoError(t, err)
					normalized, err := server.UpgradeResourceState(context.Background(), &tfprotov5.UpgradeResourceStateRequest{TypeName: "jamfpro_static_mobile_device_group", Version: 1, RawState: &tfprotov5.RawState{JSON: wantJSON}})
					require.NoError(t, err)
...
func TestSetReorderingAndChanges(t *testing.T) {
	for _, tc := range []struct {
		...
		{"legacy list order", resourceV0(), true}, {"set order", ResourceJamfProStaticMobileDeviceGroups(), false},
	} {
...
func TestExportOfflineFixture(t *testing.T) {
	...
	require.NoError(t, err)
	state := map[string]any{"version": 4, "terraform_version": "1.14.6", "serial": 1, "lineage": "00000000-0000-4000-8000-000000000042", "outputs": map[string]any{}, "resources": []any{map[string]any{"mode": "managed", "type": "jamfpro_static_mobile_device_group", "name": "test", "provider": "provider[\"registry.terraform.io/deploymenttheory/jamfpro\"]", "instances": []any{map[string]any{"schema_version": 0, "attributes": json.RawMessage(attrs), "sensitive_attributes": []any{}}}}}}
	encoded, err := json.MarshalIndent(state, "", "  ")
	require.NoError(t, err)
	require.NoError(t, os.WriteFile(filepath.Join(dir, "terraform.tfstate"), encoded, 0600))
	reverseCollections(input)
	config := map[string]any{"resource": map[string]any{"jamfpro_static_mobile_device_group": map[string]any{"test": input}}}
	encoded, err = json.MarshalIndent(config, "", "  ")
	require.NoError(t, err)
	require.NoError(t, os.WriteFile(filepath.Join(dir, "main.tf.json"), encoded, 0600))
}

func TestConstructorSetMembershipAndOmittedConfig(t *testing.T) {
	r := ResourceJamfProStaticMobileDeviceGroups()
	for _, input := range []map[string]any{setFixture(), {"name": "empty", "assigned_mobile_device_ids": []any{}}} {
		data := schema.TestResourceDataRaw(t, r.Schema, input)
		data.SetId("42")
		state := data.State()
		raw, err := state.AttrsAsObjectValue(r.CoreConfigSchema().ImpliedType())
		require.NoError(t, err)
		configAttributes := raw.AsValueMap()
		if len(input["assigned_mobile_device_ids"].([]any)) == 0 {
			configAttributes["assigned_mobile_device_ids"] = cty.SetValEmpty(cty.Number)
		}
		state.RawConfig = cty.ObjectVal(configAttributes)
		payload, err := construct(r.Data(state))
		require.NoError(t, err)
		require.NotNil(t, payload.MobileDevices)
		require.Len(t, *payload.MobileDevices, len(input["assigned_mobile_device_ids"].([]any)))
		attributes := raw.AsValueMap()
		attributes["assigned_mobile_device_ids"] = cty.NullVal(raw.GetAttr("assigned_mobile_device_ids").Type())
		state.RawConfig = cty.ObjectVal(attributes)
		payload, err = construct(r.Data(state))
		require.NoError(t, err)
		require.Nil(t, payload.MobileDevices)
	}
}
