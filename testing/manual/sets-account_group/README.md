...
Convert `member_ids` to sets in `account_group`. Add schema version 1 with the complete frozen v0 schema and a state upgrader; preserve all unrelated attributes. Singleton and ordered lists remain lists.

No spontaneous API ordering drift was reproduced: the old provider replan was clean. The old list still reports changes when only HCL member order changes; this was demonstrated with a real saved state and refresh disabled. Live migration, reorder and membership replacement were tested using temporary disabled Custom accounts.
