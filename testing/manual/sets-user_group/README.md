...
Convert `assigned_user_ids, user_additions and user_deletions` to sets in `user_group`. Add schema version 1 with the complete frozen v0 schema and a state upgrader; preserve all unrelated attributes. Singleton and ordered lists remain lists. Handle additions/deletions as submitted commands, retain readback state, avoid replay on reorder/unrelated updates, and preserve membership when the SDK serializes the request.

Draft: desired-membership migration was tested live. Addition/deletion commands were tested live only with the new provider; legacy command-state migration is covered by JSON/flatmap unit tests, not a successful legacy live apply. The legacy constructor asserts an integer for the string ID field. Command mode now preserves submitted operations and API membership; removing a command does not undo it. Explicit assigned_user_ids cannot be combined with operation blocks.
